...
public class TimerMetricRecorderTest
{
	private static final String ENABLED = "cloud.commons.monitoring.metric.timer.enabled";
	...
	private static final String STEP_METRIC = String.format("tenant=%s,extension=%s,area=%s,step=%s,name=%s",
			TENANT_ID, CloudCommonsConstants.EXTENSIONNAME, AREA.name(), STEP.name(), STEP_STATUS.name() + ".timer");

	@Spy
	private TimerMetricRecorder recorder = new TimerMetricRecorder();
	...
	public void setUp()
	{
		...
		given(metricRegistry.timer(STEP_METRIC)).willReturn(stepMetric);
	...
	public void shouldRecordDurationOfUnsavedSavedEntries() {
		...
		verify(stepMetric).update(5L, TimeUnit.MILLISECONDS);
	}
	...
	public void shouldRecordCalculatedDurationOfUnsavedSavedEntries() {
		...
		verify(stepMetric).update(duration, TimeUnit.MILLISECONDS);
	}
	...
	public void shouldNotRecordUnsavedSavedEntryWhereHasNoDuration() {
		...
		verify(stepMetric, never()).update(anyLong(), eq(TimeUnit.MILLISECONDS));
	}
}
