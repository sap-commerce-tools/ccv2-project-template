...
public class CounterMetricRecorderTest
{
	private static final String ENABLED = "cloud.commons.monitoring.metric.counter.enabled";
	...
	private static final String STATUS_METRIC = String.format("tenant=%s,extension=%s,area=%s,name=%s",
			TENANT_ID, CloudCommonsConstants.EXTENSIONNAME, AREA.name(), STATUS.name() + ".counter");

	private static final String STEP_METRIC = String.format("tenant=%s,extension=%s,area=%s,step=%s,name=%s",
			TENANT_ID, CloudCommonsConstants.EXTENSIONNAME, AREA.name(), STEP.name(), STEP_STATUS.name() + ".counter");

	@Spy
	private CounterMetricRecorder recorder = new CounterMetricRecorder();
	...
	@Mock
	private Counter statusMetric;

	@Mock
	private Counter stepMetric;
	...
	public void setUp()
	{
		...
		given(metricRegistry.counter(STATUS_METRIC)).willReturn(statusMetric);
		given(metricRegistry.counter(STEP_METRIC)).willReturn(stepMetric);
	...
	@Test
	public void shouldRecordMetricForStatusOfHistory()
	{
		recorder.record(history);

		verify(statusMetric).inc();
		verifyNoMoreInteractions(statusMetric);
	}

	@Test
	public void shouldRecordMetricForStatusAndOnOriginalStatusOfHistory()
	{
		history.setStatusWhenResumed(Status.IN_PROGRESS);

		final Counter inProgressCounter = mock(Counter.class);
		final String inProgressKey = String.format("tenant=%s,extension=%s,area=%s,name=%s",
				TENANT_ID, CloudCommonsConstants.EXTENSIONNAME, AREA.name(), Status.IN_PROGRESS.name() + ".counter");
		given(metricRegistry.counter(inProgressKey)).willReturn(inProgressCounter);

		recorder.record(history);

		verify(statusMetric).inc();
		verify(inProgressCounter).dec();
		verifyNoMoreInteractions(statusMetric, inProgressCounter);
	}
	...
	@Test
	public void shouldRecordHistoryOfUnsavedSavedEntries()
	{
